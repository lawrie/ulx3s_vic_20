# micropython ESP32
# VIC20 PRG loading from web, proof of concept
...
# init file browser
  ...
  def change_file(self):
    ...
    if filename:
      ...
      if filename.endswith(".prg"):
        stream=self.open_web(self.url+filename.replace(" ","%20"))
        self.enable[0]=0
        self.osd_enable(0)
        import ld_vic20
        s=ld_vic20.ld_vic20(self.spi,self.cs)
        s.loadprg_stream(stream)
        del s
        gc.collect()
  ...
  def read_dir(self):
    self.direntries = []
    index = self.open_web(self.url+"/index.html")
    if index:
      while True:
        s=index.readline().decode()
        if s:
          if s.find("<dt><a href=")>=0 and s.find(".prg")>=0:
            n1 = s.split('">')
            if len(n1)>1:
              n2 = n1[1].split("</a>")
              if len(n2)>1:
